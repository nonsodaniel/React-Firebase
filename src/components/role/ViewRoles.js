...
class ViewRoles extends Component {
  ...
  fetchRoles = async () => {
    ...
    // isDev ? "http://localhost:4000/api" : "http://www.acadatrends.com/api"
    fetch(`${base_url}/roles/`)
      .then(response => {
        return response.json();
      })
      .then(rolesData => {
        console.log('yes roles', rolesData.data);
        this.setState({
          category: rolesData.data.length ? rolesData.data.reverse() : null
        });
        $('#example').DataTable();
      });
  };
